...
playGame ={
	...
	game.load.image("one","img/l1.png");
    ...
    update:function() {
    	...
    	game.physics.arcade.collide(platform1,aii);
    	game.physics.arcade.overlap(bullets,atback,s);
	...
	function fireBullet()
{
        ...
        function s(bullets,atback)
{
	alert.text = "MOVE TO NEXT LEVEL \n 	OR RESTART!";
	next = game.add.button(250,350,"retry",retry);
    
        }
		function likod()
{
	back = game.add.sprite(aii.position.x + 150,aii.position.y + 50,"back");
...
{
	bullets.kill();
	next = game.add.button(450,350,"next",process.start);
